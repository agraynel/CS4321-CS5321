...
public class EvaluationMklll extends EvaluationMkll implements ExpressionVisitor {
	 ...
	 * @param the number of pages that will be used for external sorting.
	 */
	public EvaluationMklll(Map<String, SelectOperator> map, String[] array,
			Map<String, Tuple> map2, Map<String, String> hash, Map<String, Integer> location, int pages) {
	...
	public void visit(Parenthesis node) {
		...
		if(length==1){
			...
			if(pages==0) op = new SortOperator(seo1,dummy1,seo1.getLocation());
			else op = new EXSortOperator(seo1,dummy1,seo1.getLocation(),pages,0);
			MoveBack ex2 = null;
			if(pages==0) ex2 = new SortOperator(seo2,dummy2,seo2.getLocation());
			else ex2 = new EXSortOperator(seo2,dummy2,seo2.getLocation(),pages,1);
			...
			for(int i=0;i<temp2.size();i++)
				dummy2[i] = temp2.get(i);
			if(pages==0) op = new SortOperator(op,dummy1,location);
			else op = new EXSortOperator(op,dummy1,location,pages,length*2-2);
			SelectOperator dummy = map.get(array[length]);
			MoveBack ex2 = null;
			if(pages==0) ex2 = new SortOperator(dummy,dummy2,dummy.getLocation());
			else ex2 = new EXSortOperator(dummy,dummy2,dummy.getLocation(),pages,length*2-1);
			file = new File(Interpreter.getTemp() + "dummy " + length);
			merge(op,ex2,dummy1,dummy2,location,dummy.getLocation());
			op = new ScanOperator(file);
			marked.add(array[length]);
		}
	}

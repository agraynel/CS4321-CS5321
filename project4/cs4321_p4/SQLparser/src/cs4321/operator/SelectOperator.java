...
public class SelectOperator extends Operator{
	 ...
	 */
	public SelectOperator(File file, String string, Expression express, Map<String,String> hash, double[] temp, int mark){
		if(temp[0]==Double.NEGATIVE_INFINITY&&temp[1]==Double.POSITIVE_INFINITY)
			scan = new ScanOperator(file);
		else scan = new IndexScanOperator(hash.get(string),catalog.getIndex(hash.get(string)),
				         temp[0],temp[1],mark);

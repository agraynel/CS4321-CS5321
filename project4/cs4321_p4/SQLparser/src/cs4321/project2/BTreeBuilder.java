...
public class BTreeBuilder {
	...
	public void dump(){
		...
		String output = Interpreter.getOutput() + "/BTree ";
		...
		try{
			...
			while((s=buff.readLine())!=null){
				...
				int cluster = catalog.getCluster(str[0]);
				int order = catalog.getOrder(str[0]);
				construct(str[0],str[1],point,cluster,order,output+str[0]);
			}
			buff.close();
		}catch(Exception e){
			e.printStackTrace();
	 ...
	 * @param temp the file channel handles the jumping of positions
	 * @param order the order of the B+ Tree.

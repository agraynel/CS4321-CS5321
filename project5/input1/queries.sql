...
WHERE A.A = B.A AND B.B > 7800 AND B.B < 7500 AND A.C > 9050
ORDER BY A.B;

SELECT Boats.E, Sailors.C
FROM Sailors, Boats 
WHERE Sailors.B = Boats.F AND Sailors.A > 9000 AND Boats.E < 100 AND Boats.D != 1200 AND Sailors.C <= 2400 AND Sailors.A < 9600
ORDER BY Sailors.C;

SELECT * FROM Sailors WHERE Sailors.B < 300 AND Sailors.C > 7900
ORDER BY Sailors.B;
...
WHERE B.F >= 3000 AND B.F < 5000 AND S1.A = B.D AND S1.A < 2000 AND S1.C > 9300 AND S2.C = B.F AND S2.B < 6666 AND S2.A > 7888
ORDER BY B.D, S2.A;

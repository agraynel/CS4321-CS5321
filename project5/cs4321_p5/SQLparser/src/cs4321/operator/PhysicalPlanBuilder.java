...
public class PhysicalPlanBuilder {
	...
	public void visit(JoinOperators operator, String s, StringBuilder str){
		...
		String[] collect = new String[array.length];
		for(int i=0;i<collect.length;i++)
			collect[i] = array[i];
		if(array.length>2&&!ps.getSelectItems().get(0).toString().equals("*")){
			DP dp = new DP(operator,array,hash);
			List<String> order = dp.returnOrder();
			for(int i=0;i<array.length;i++)
				array[i] = order.get(i);
		}
		String[] collection = fetchJoins();
	...
	private int build(double[][] list, String table, double[][] store) {
		File file = new File(Interpreter.getOutput() + "/statistics.txt");
	...
	/**
	 * get a list of conditions that are join conditions and store them in an array.
	 * @return an array of Join conditions.
	 */
	private String[] fetchJoins() {
		List<String> temp = new ArrayList<>();
		Expression where = ps.getWhere();
		if(where!=null){
			String[] temp1 = where.toString().split("AND");
			for(int i=0;i<temp1.length;i++){
				String dummy = temp1[i].trim();
				String[] str1 = dummy.split("\\s+");
				String[] str2 = str1[0].split("\\.");
				String[] str3 = str1[2].split("\\.");
				if(!isNumber(str2[0])&&!isNumber(str3[0])&&!str2[0].equals(str3[0]))
					temp.add(dummy);
			}
		}

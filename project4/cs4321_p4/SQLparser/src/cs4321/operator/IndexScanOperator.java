...
public class IndexScanOperator extends Operator{
	 ...
	 */
	public IndexScanOperator(String table, String column, double low, double high, int mark) {
		this.high = high;
		this.table = table;
		...
		int order = catalog.getOrder(table);
		String output = Interpreter.getTemp() + "/BTree " + table;
		if(mark==1) file = BTree.construct(table, column, point, cluster, order,output);
		else file = new File(Interpreter.getInput()+"/db/indexes/" + table + "." + column);
		traverse(file,low);
		try {

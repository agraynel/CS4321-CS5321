...
public class EvaluationMkll implements ExpressionVisitor {
	...
	private Map<String,SelectOperator> map;
	private Map<String,Tuple> map2;
	private Map<String,String> hash; 
	// this map links the aliases to their original tables.
	private int length; // the index of the array.
	private Catalog catalog;
	private boolean stillLeft = true, isValid;
	...
	/**
	 * check whether there are still valid tuples left.
	 * @return the state "there are still tuples left".
	 */
	...
	public void visit(Parenthesis node) {
        ...
        if(length==0){
        	map2.put(array[length],map.get(array[length]).nextTuple());
